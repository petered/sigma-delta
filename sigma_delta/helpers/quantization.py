...
def sequential_quantize(v, n_steps = None, method='herd', rng = None):
    ...
    if method=='herd':
        result = np.diff(np.round(np.cumsum(np.insert(v, 0, 0, axis=-2), axis=-2)), axis=-2)
    elif method=='round':
        result = np.round(v)
    elif method == 'slippery.9':
        return slippery_round(v, slip=0.9)
    elif method == 'slippery.5':
        return slippery_round(v, slip=0.5)
    elif method=='uniform':
        result = v + rng.uniform(-.5, .5, size=v.shape)
    elif method=='surrogate-noise':
        return v + (12**.5)*((v%1)-(v%1)**2)*rng.uniform(low=-.5, high=.5, size=v.shape)
    elif method == 'surrogate-sqrt':
        return v + np.sqrt((12**.5)*((v%1)-(v%1)**2)*rng.uniform(low=-.5, high=.5, size=v.shape))
    elif method is None:
        result = v
    else:
